...
#     #
#     ce = ContentExtractor('http://www.baidu.com/link?url=yODTDqSmPWEM8jki-6VLgzQqXZUHYHjiuj4GiFziPi3')
...
def ExtractorToData(url):
    ce = ContentExtractor(url)
    ce.execute()
    # print codeSet.setCharSet(ce.title)
    # print codeSet.setCharSet(ce.content)

    # print ce.title
    # print ce.content
    storeFinFlie(ce.title, ce.content)
    return ce.content


def storeFinFlie(title,content):
    f=open("dataflie/"+title.encode("GB18030",'ignore')+".txt",'wb')
    f.write(content.encode("GB18030",'ignore'))
    print 'file tree is building...'
    print title+" is modifily and stored in one tree node...."



# url =r'http://python.jobbole.com/81065/'
# ExtractorToData(url)